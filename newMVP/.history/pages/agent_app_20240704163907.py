...
class AITaskOrchestrator:
    ...
    def set_environment(self):
        ...
        gemini_api_key = random.choice(self.KEYS)
    ...
    def gpt_orchestrator(self, objective, file_content=None, previous_results=None, use_search=False):
    ...
    def gpt_sub_agent(self, prompt, search_query=None, previous_gpt_tasks=None, use_search=False, continuation=False):
        ...
        if len(response_text) >= 4000:
            ...
            continuation_response_text = self.gpt_sub_agent(prompt, search_query, previous_gpt_tasks, use_search, continuation=True)
            response_text += continuation_response_text

        return response_text

    def anthropic_refine(self, objective, sub_task_results, filename, projectname, continuation=False):
        ...
        if len(response_text) >= 4000 and not continuation:
            st.warning("Output may be truncated. Attempting to continue the response.")
            continuation_response_text = self.anthropic_refine(objective, sub_task_results + [response_text], filename, projectname, continuation=True)
            response_text += "\n" + continuation_response_text

        return response_text

    def create_folder_structure(self, project_name, folder_structure, code_blocks):
        st.write(f"Creating project folder: {project_name}")
        zip_buffer = io.BytesIO()
        with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
            self.create_folders_and_files_recursive(zip_file, project_name, folder_structure, code_blocks)
        return zip_buffer

    def create_folders_and_files_recursive(self, zip_file, current_path, structure, code_blocks):
        for key, value in structure.items():
            path = os.path.join(current_path, key)
            if isinstance(value, dict):
                st.write(f"Created folder: {path}")
                self.create_folders_and_files_recursive(zip_file, path, value, code_blocks)
            else:
            ...
            with st.spinner("Processing your task..."):
                while True:
                    ...
                    if not task_exchanges:
                        gpt_result, file_content_for_gpt, search_query = self.gpt_orchestrator(objective, file_content, previous_results, use_search)
                    else:
                        gpt_result, _, search_query = self.gpt_orchestrator(objective, previous_results=previous_results, use_search=use_search)
                    ...
                    if "The task is complete:" in gpt_result:
                        ...
                        sub_task_result = self.gpt_sub_agent(sub_task_prompt, search_query, gpt_tasks, use_search)
                ...
                refined_output = self.anthropic_refine(objective, sub_task_results, timestamp, sanitized_objective)
                ...
                zip_buffer = self.create_folder_structure(project_name, folder_structure, code_blocks)
