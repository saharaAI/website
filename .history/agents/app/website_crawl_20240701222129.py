...
# Configure genai
genai.configure(api_key=KEYS[0])  # Use the first key by default
...
class WebScraper:
    def __init__(self, url):
        ...
        self.extracted_text = ""
        self.metadata = {}
        self.crawled_links = [] # Store links found during crawling

    def scrape(self):
        try:
            downloaded = trafilatura.fetch_url(self.url)
            self.extracted_text = trafilatura.extract(downloaded)
            self.metadata = trafilatura.metadata.extract_metadata(downloaded)
        except Exception as e:
            st.error(f"An error occurred during scraping: {e}")

    def crawl(self, max_urls=30): 
        try:
            self.crawled_links, _ = trafilatura.spider.focused_crawler(
                self.url, max_seen_urls=max_urls
            )
        except Exception as e:
...
def main():
    ...
    if st.button("Scrape and Analyze"):
        if url:
            # Scrape and Crawl the website
            scraper = WebScraper(url)
            scraper.scrape()
            scraper.crawl() # Crawl the site 

            # Display extracted content, metadata, and crawled links
            ...
            if scraper.crawled_links:
                ...
                for link in scraper.crawled_links:
                    st.write(link)
