...
class GeminiApp:
    ...
    def generate_response(self, user_input: str, context: str, history: List[Dict[str, str]]) -> str:
        ...
        full_prompt = f"You are a helpful and informative chatbot. Here's the context from multiple web pages:\n\n{context[:5000]}\n\nUser question: {user_input}\n\nAnswer:"
...
def main():
    ...
    if st.button("Scrape, Crawl, and Analyze"):
        if url:
            ...
            gemini = GeminiApp()
            response = gemini.generate_response(prompt, scraper.all_text, st.session_state.chat_history)
