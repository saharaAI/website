...
class WebScraper:
    ...
    def crawl(self, max_urls: int = MAX_URLS):
        try:
            with st.spinner('Crawling in progress...'):
                ...
                with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                    future_to_link = {executor.submit(self._process_link, link): link for link in self.crawled_links}
...
class GeminiApp:
    ...
    def generate_response(self, context: str, num_urls: int) -> str:
        ...
        prompt = f"""
        You are an expert data analyst, finance expert, and report generator. You have been given the content from {num_urls} web pages. Your task is to analyze this content and provide a comprehensive report.

        Context (excerpt from scraped web pages):
        {context}

        Please generate (ALWAYS: IN FRENCH) a report with the following structure:
        ...
        2. Table of key topics as table markdown: 
        ...
        Begin your report with "Summary:" and end it with "JSON Output:"
        """
...
def main():
    st.set_page_config(page_title="Enhanced Web Scraper & Gemini App", layout="wide")
    st.title("Enhanced Web Scraper & Gemini App")

    url = st.text_input("Enter the URL of the website:", value="https://www.bpm.mr/")
    ...
    if st.button("Scrape, Crawl, and Analyze"):
        if url:
            ...
            if len(parts) == 2:
                french_summary, json_part = parts
                st.subheader("Summary (in French)")
                st.write(french_summary.strip())
