...
st.set_page_config(page_title="AI Task Orchestrator", layout="wide")

# Sidebar for API key inputs
st.sidebar.header("API Keys")
openai_api_key = st.sidebar.text_input("OpenAI API Key", type="password")
anthropic_api_key = st.sidebar.text_input("Anthropic API Key", type="password")
gemini_api_keys = st.sidebar.text_area("Gemini API Keys (one per line)", height=100)
...
def create_folder_structure(project_name, folder_structure, code_blocks):
    ...
    for key, value in folder_structure.items():
        if isinstance(value, dict):
            st.write(f"Created folder: {key}")
            create_folder_structure(os.path.join(project_name, key), value, code_blocks)
        else:
            code_content = next((code for file, code in code_blocks if file == key), None)
            if code_content:
                st.write(f"Created file: {key}")
