...
def copy_to_clipboard(text, button_label="Copier"):
    # Créer un ID unique pour chaque bouton
    button_id = f"copy_button_{hash(text)}"
    
    # Créer le HTML et le JavaScript pour le bouton de copie
    st.markdown(f"""
    <textarea id="text_{button_id}" style="position: absolute; left: -9999px;">{text}</textarea>
    <button id="{button_id}" onclick="copyText('{button_id}')">{button_label}</button>
    <script>
    function copyText(buttonId) {{
        var textArea = document.getElementById('text_' + buttonId);
        textArea.select();
    ...
    for i, message in enumerate(st.session_state.chat_history):
        with st.chat_message(message["role"]):
            ...
            if message["role"] == "model":
                ...
                with col1:
                    copy_to_clipboard(message['parts'][0], "📋 Copier")
                with col2:
