...
class Ludo:
    ...
    def move_token(self, player_id, token_index, steps):
        ...
        if start == -1:  # Move out of start
            new_position = self.players[player_id]['start']
        else:
            new_position = self.get_new_position(start, steps)
        
        if new_position not in self.safe_spots:
            self.board[start] = None  # Clear the old position
            self.board[new_position] = player_id  # Update the new position
    ...
    def play_turn(self, player_id):
        ...
        print(f"Player {player_id} rolls a {dice_roll}")
    ...
    def display_board(self):
        for player_id, player_data in self.players.items():
            print(f"Player {player_id} tokens: {player_data['tokens']}")

    def start_game(self):
        num_turns = int(input("Enter the number of turns to play: "))
