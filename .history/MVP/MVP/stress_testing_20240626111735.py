...
if st.button("Run Stress Test"):
    ...
    for i, scenario in enumerate(scenarios):
        ...
        # --- Display Scenario Adjustments ---
        st.markdown(f"Interest Rate Change: {scenario['interest_rate_change']:.2%}")
        st.markdown(f"Unemployment Change: {scenario['unemployment_change']:.2%}")
        st.markdown(f"GDP Growth Change: {scenario['gdp_growth_change']:.2%}")
        st.markdown(f"Inflation Change: {scenario['inflation_change']:.2%}")
