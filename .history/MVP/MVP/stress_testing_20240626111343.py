...
class StressTester:
    # ... (Existing __init__ and simulate_default_probability methods) ...
...
if st.button("Run Stress Test"):
    ...
    for i, scenario in enumerate(scenarios):
        ...
        st.write(f"Total Estimated Loss: ${results[f'loss_{scenario['name']}'].sum():,.2f}")
        st.write(f"Mean Loss Rate: {results[f'loss_{scenario['name']}'].sum() / results['loan_amount'].sum() * 100:.2f}%")
        st.write(f"Simulated Default Rate: {results[f'default_{scenario['name']}'].mean() * 100:.2f}%")
