...
class MultimodalAIChat:
    ...
    def chat_with_gemini(self, user_input: str, image_path=None, current_iteration=None, 
                       max_iterations=None, update_function=None):
        """Interacts with Google Gemini using a chat session and handles updates.

        Args:
            user_input (str): The user's text input.
            image_path (str, optional): Path to an image (not used currently).
            current_iteration (int, optional): Current iteration in automode.
            max_iterations (int, optional): Maximum iterations in automode.
            update_function (function, optional): A function to call for updates.
                It should take arguments: iteration (int), message (str).

        Returns:
            tuple: (response, exit_continuation) 
                response (str): The AI's response.
                exit_continuation (bool): True if automode should exit.
        """

        if image_path:
            self.print_colored("Gemini doesn't support images yet. Please provide text input.", self.TOOL_COLOR)
            return "I'm sorry, I cannot process images at this time. Please provide text input.", False

        context = ""  # You might need to manage context differently
        ...
        if update_function is not None:
            update_function(iteration=current_iteration, message=response)
