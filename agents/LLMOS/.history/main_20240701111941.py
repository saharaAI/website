...
class MultimodalAIChat:
    ...
    def chat_with_gemini(self, user_input, image_path=None, current_iteration=None, max_iterations=None):
        if image_path:
            # Gemini doesn't natively support images yet, so you can either:
            # 1. Use an image captioning API to describe the image and add it to the text
            # 2. Wait for Gemini to support images directly
        ...
        response = self.gemini_model.predict(
            context=self.update_system_prompt(current_iteration, max_iterations),
            messages=self.conversation_history
        )

        self.print_colored(f"\nGemini: {response.text}", self.AI_COLOR)
        self.conversation_history.append({"role": "assistant", "content": response.text})

        if self.CONTINUATION_EXIT_PHRASE in response.text:
